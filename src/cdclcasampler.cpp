...
void CDCLCASampler::SetDefaultPara()
{
    ...
    final_strategy = 0;
    p_choose_final = &CDCLCASampler::new_choose_final_plain;
}
...
void CDCLCASampler::Setcount(int count)
{
    trans_count_ = count;
}
...
{
    vector<pair<float, float>> sample_prob1 = get_prob_in1(false);
...
void CDCLCASampler::GenerateCoveringArray()
{
    ...
    for (num_generated_testcase_ = 1; ; num_generated_testcase_++)
    {
        ...
        GenerateTestcase();
        if (selected_candidate_bitset_.count()<trans_count_){
...
vector<pair<float, float> > CDCLCASampler::get_prob_in1(bool init){
...
std::unordered_map<int, int> CDCLCASampler::difference_(const std::unordered_map<int, int>& map1, const std::unordered_map<int, int>& map2) {  

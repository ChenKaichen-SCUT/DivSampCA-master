...
public:
    ...
    std::vector<pair<float, float>> sample_proportion;
    std::vector<pair<float, float>> get_prob_in1(bool init);
    void new_choose_final_plain();
    std::unordered_map<int, int> difference_(const std::unordered_map<int, int>& map1, const std::unordered_map<int, int>& map2); 
    void print_vector(vector<int> tc);
    void print_vector_vector(vector<vector<int>> tc);
    void print_unoredered_map(unordered_map<int,int> tc);
